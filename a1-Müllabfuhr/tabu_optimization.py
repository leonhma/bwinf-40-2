...
def MMKCPP_TEE_TabuSearch(G: Dict[int, Dict[int, float]], tours: List[Tuple[int, ...]], /, maxNOfItsWithoutImprovement: int = 100,
    ...
    for start in dijkstra:
        ...
        while q:
            ...
            dijkstra[start][current] = (length, tuple(currentpath[1:]) if len(currentpath) else tuple(currentpath))
    ...
    def MergeWalkWithTour(tour: Tuple[int, ...], walk: Tuple[int, ...]) -> Tuple[int, ...]:
        ...
        # splice
        return tour[:min_idx]+('debug',)+min_sp_u+('debug',)+walk+('debug',)+min_sp_v+('debug',)+tour[min_idx:]
    ...
    def SeparateWalkFromTour(tour: Tuple[int, ...], walk: Tuple[int, ...]) -> Tuple[int, ...]:
        ...
        if li == 0:
            return dijkstra[0][v][1]+tour[ri:]+dijkstra[tour[-1]][0][1]
        if ri == len(tour)-1:
            return dijkstra[0][tour[0]][1]+tour[:ri+1]+dijkstra[u][0][1]
