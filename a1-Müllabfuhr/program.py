...
class CityGraph:
    ...
    def get_paths(self, days: int = 5) -> List[Tuple[float, Tuple[int, ...]]]:
        return map(lambda x: (self.w_tour(x), x), MMKCPP_TEE_TabuSearch(self.vertices, days, maxNOfItsWithoutImprovement=100, maxRunningTime=min(300, 0.1*len(self.vertices)**2)))
